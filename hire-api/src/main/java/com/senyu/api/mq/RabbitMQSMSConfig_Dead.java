...
public class RabbitMQSMSConfig_Dead {
    ...
    // 统一定义路由key
    public static final String ROUTING_KEY_SMS_SEND_DEAD = "dead.sms.display";
    // 创建交换机
    ...
    @Bean(SMS_QUEUE_DEAD)
    public Queue queue() {
        return QueueBuilder.durable(SMS_QUEUE_DEAD).withArgument("x-message-ttl", 10 * 1000).build();
    }
    // 绑定队列和交换机
    @Bean
